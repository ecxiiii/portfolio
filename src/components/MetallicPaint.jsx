...
export function MetallicPaint({
  imageSrc, seed=42, scale=4, refraction=0.01, blur=0.015, liquid=0.75,
  speed=0.3, brightness=2, contrast=0.5, angle=0, fresnel=1,
  ...
  const uploadTexture = useCallback(imgData => {
    const gl=glRef.current, u=uniformsRef.current;
  ...
  useEffect(() => {
    if (!initGL()) return;
    const canvas=canvasRef.current, gl=glRef.current, side=1000*devicePixelRatio;
    canvas.width=side; canvas.height=side; gl.viewport(0,0,side,side);
    setReady(true);
    return () => { if(rafRef.current) cancelAnimationFrame(rafRef.current); if(textureRef.current&&glRef.current) glRef.current.deleteTexture(textureRef.current); };
  }, [initGL]);
  ...
  useEffect(() => {
    ...
    const gl=glRef.current, u=uniformsRef.current, canvas=canvasRef.current, mouse=mouseRef.current;
    const handleMouseMove = e => { const rect=canvas.getBoundingClientRect(); mouse.targetX=(e.clientX-rect.left)/rect.width; mouse.targetY=(e.clientY-rect.top)/rect.height; };
